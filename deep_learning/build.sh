...
# ./build.sh all      -> thêm PDF riêng cho từng phần, từng chương, từng mục
...
case "$1" in
  "")   build deep_learning.tex ;;
  all)  build deep_learning.tex
        build 00-map.tex
        for f in [A-F]-*/*.tex [A-F]-*/*/*.tex [A-F]-*/*/*/*.tex; do build "$f"; done ;;
  *)    build "$1" ;;
esac
