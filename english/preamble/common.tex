% ============================================================================
%  preamble/common.tex — style dùng chung cho toàn cây english.
...
% ---------- CẶP ĐÔI: câu trước / câu sau khi sửa ----------
% \begin{capdoi} \truoc{...} \sau{...} \vico{vì sao} ... \end{capdoi}
\newenvironment{capdoi}[1][Trước và sau]
 {\par\medskip\bmblock{#1}%
  \noindent{\raisebox{0.06em}{\color{accent}\rule{2.6pt}{0.82em}}\hspace{0.5em}%
   \sffamily\bfseries\color{inkblue}#1}\par\vspace{0.4em}\small}
 {\par\medskip}
\newcommand{\cdrow}[3]{\par\vspace{0.16em}\noindent
  \hangindent=3.3em\hangafter=1
  \makebox[3.2em][l]{\footnotesize\sffamily\bfseries\color{#1}#2}#3\par}
\newcommand{\truoc}[1]{\cdrow{reddark}{Trước}{#1}}
\newcommand{\sau}[1]{\cdrow{greendark}{Sau}{#1}}
\newcommand{\vico}[1]{\cdrow{tiercol}{Vì sao}{\footnotesize\color{tiercol}#1}\vspace{0.35em}}

% ---------- BẢNG SẮC THÁI TỪ ----------
% \begin{tuvung} \tv{word}{nghĩa lõi}{sắc thái / văn vực}{ví dụ} ... \end{tuvung}
\newenvironment{tuvung}[1][Sắc thái của các từ gần nghĩa]
 {\par\medskip\bmblock{#1}%
  \noindent{\raisebox{0.06em}{\color{accent}\rule{2.6pt}{0.82em}}\hspace{0.5em}%
   \sffamily\bfseries\color{inkblue}#1}\par
  \vspace{0.45em}\footnotesize
  \rowcolors{2}{white}{tblalt}%
  \longtable{@{}L{2.5cm}L{3.4cm}L{3.6cm}L{5.4cm}@{}}
  \toprule
  \textbf{Từ} & \textbf{Nghĩa lõi} & \textbf{Sắc thái / văn vực} & \textbf{Ví dụ dùng đúng}\\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Từ} & \textbf{Nghĩa lõi} & \textbf{Sắc thái / văn vực} & \textbf{Ví dụ (tiếp)}\\
  \midrule
  \endhead}
 {\bottomrule\endlongtable\par\medskip}
\newcommand{\tv}[4]{\textbf{#1} & #2 & #3 & \emph{#4}\\}

% ---------- MẪU CÂU DÙNG LẠI ĐƯỢC ----------
% \begin{mauau} \mc{chức năng}{mẫu câu}{ghi chú} ... \end{mauau}
\newenvironment{mauau}[1][Mẫu câu dùng lại được]
 {\par\medskip\bmblock{#1}%
  \noindent{\raisebox{0.06em}{\color{accent}\rule{2.6pt}{0.82em}}\hspace{0.5em}%
   \sffamily\bfseries\color{inkblue}#1}\par
  \vspace{0.45em}\footnotesize
  \rowcolors{2}{white}{tblalt}%
  \longtable{@{}L{3.4cm}L{7.4cm}L{4.0cm}@{}}
  \toprule
  \textbf{Chức năng} & \textbf{Mẫu câu} & \textbf{Ghi chú}\\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Chức năng} & \textbf{Mẫu câu} & \textbf{Ghi chú (tiếp)}\\
  \midrule
  \endhead}
 {\bottomrule\endlongtable\par\medskip}
\newcommand{\mc}[3]{#1 & \emph{#2} & {\footnotesize\color{tiercol}#3}\\}
